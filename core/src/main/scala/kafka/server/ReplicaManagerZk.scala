...
import kafka.server.HostedPartitionZk.Online
import kafka.server.QuotaFactory.QuotaManagers
import kafka.server.checkpoints.{LazyOffsetCheckpoints, OffsetCheckpointFile, OffsetCheckpoints}
import kafka.server.metadata.{ConfigRepository, MetadataBroker, ZkConfigRepository}
 ...
 */
sealed trait HostedPartitionZk
object HostedPartitionZk {
  /**
   * This broker does not have any state for this partition locally.
   */
  final object None extends HostedPartitionZk

  /**
   * This broker hosts the partition and it is online.
   */
  final case class Online(partition: Partition) extends HostedPartitionZk

  /**
   * This broker hosts the partition, but it is in an offline log directory.
   */
  final object Offline extends HostedPartitionZk
}

class ReplicaManagerZk(val config: KafkaConfig,
                       metrics: Metrics,
                       time: Time,
  ...
  def this(config: KafkaConfig,
    ...
    this(config, metrics, time, zkClient, scheduler, logManager, isShuttingDown,
      ...
      threadNamePrefix, configRepository, alterIsrManager, helper)
  }
  ...
  def this(config: KafkaConfig,
           ...
           helper: ReplicaManagerHelper = new ReplicaManagerHelper()) = {
    this(config, metrics, time, Some(zkClient), scheduler, logManager, isShuttingDown,
      ...
      new ZkConfigRepository(new AdminZkClient(zkClient)), threadNamePrefix, helper)
  }

  /* epoch of the controller that last changed the leader */
  @volatile var controllerEpoch: Int = KafkaController.InitialControllerEpoch
  private val localBrokerId = config.brokerId
  private val allPartitions = new Pool[TopicPartition, HostedPartitionZk](
    valueFactory = Some(tp => HostedPartitionZk.Online(Partition(tp, time, this)))
  )
  ...
  this.logIdent = s"[ReplicaManagerZk broker=$localBrokerId] "
  ...
  // Visible for testing
  private[kafka] val partitionCount = newGauge("PartitionCount", () => allPartitionsCount)
  ...
  override def getLog(topicPartition: TopicPartition): Option[Log] = logManager.getLog(topicPartition)
  ...
  private def maybeRemoveTopicMetrics(topic: String): Unit = {
    val topicHasOnlinePartition = allPartitions.values.exists {
      case HostedPartitionZk.Online(partition) => topic == partition.topic
      case HostedPartitionZk.None | HostedPartitionZk.Offline => false
    }
    if (!topicHasOnlinePartition)
      brokerTopicStats.removeMetrics(topic)
  }

  override def stopReplicas(correlationId: Int,
                            controllerId: Int,
                            controllerEpoch: Int,
                            brokerEpoch: Long,
      ...
      if (controllerEpoch < this.controllerEpoch) {
        ...
        partitionStates.forKeyValue { (topicPartition, partitionState) =>
          ...
          getPartition(topicPartition) match {
            case HostedPartitionZk.Offline =>
            ...
            case HostedPartitionZk.Online(partition) =>
  ...
  private def stopPartitions(partitionsToStop: Map[TopicPartition, Boolean]):
    ...
    partitionsToStop.forKeyValue { (topicPartition, shouldDelete) =>
      if (shouldDelete) {
        getPartition(topicPartition) match {
          case hostedPartition@HostedPartitionZk.Online(partition) =>
  ...
  private def getPartition(topicPartition: TopicPartition): HostedPartitionZk = {
    Option(allPartitions.get(topicPartition)).getOrElse(HostedPartitionZk.None)
  }
  ...
  def createPartition(topicPartition: TopicPartition): Partition = {
    ...
    allPartitions.put(topicPartition, HostedPartitionZk.Online(partition))
    partition
  }

  def onlinePartition(topicPartition: TopicPartition): Option[Partition] = {
    getPartition(topicPartition) match {
      case HostedPartitionZk.Online(partition) => Some(partition)
      case HostedPartitionZk.None | HostedPartitionZk.Offline => None
    }
  }

  override def onlinePartitionsIterator: Iterator[Partition] = {
  ...
  override def offlinePartitionCount: Int = {
    allPartitions.values.iterator.count(_ == HostedPartitionZk.Offline)
  }
  ...
  def onlinePartitionOrError(topicPartition: TopicPartition): Either[Errors, Partition] = {
    getPartition(topicPartition) match {
      ...
      case HostedPartitionZk.None if metadataCache.contains(topicPartition) =>
  ...
  def getLogDir(topicPartition: TopicPartition): Option[String] = {
    localOnlineLog(topicPartition).map(_.parentDir)
  }
      ...
      partitionDirs.map { case (topicPartition, destinationDir) =>
        try {
          ...
          getPartition(topicPartition) match {
            ...
            case HostedPartitionZk.None => // Do nothing
          }
  ...
  private def appendToLocalLog(internalTopicsAllowed: Boolean,
    ...
    def processFailedRecord(topicPartition: TopicPartition, t: Throwable) = {
      val logStartOffset = getPartition(topicPartition) match {
        case HostedPartitionZk.Online(partition) => partition.logStartOffset
        case HostedPartitionZk.None | HostedPartitionZk.Offline => -1L
      }
  ...
  override def maybeUpdateMetadataCache(correlationId: Int, updateMetadataRequest: UpdateMetadataRequest) : Seq[TopicPartition] =  {
    replicaStateChangeLock synchronized {
  ...
  override def becomeLeaderOrFollower(correlationId: Int,
        ...
        if (leaderAndIsrRequest.controllerEpoch < controllerEpoch) {
          ...
          requestPartitionStates.foreach { partitionState =>
            ...
            val partitionOpt = getPartition(topicPartition) match {
              ...
              case HostedPartitionZk.None =>
                val partition = Partition(topicPartition, time, this)
                allPartitions.putIfNotExists(topicPartition, HostedPartitionZk.Online(partition))
                Some(partition)
            }
          ...
          helper.maybeAddLogDirFetchers(this, partitionStates.keySet, highWatermarkCheckpoints)
  ...
  def markPartitionOffline(tp: TopicPartition): Unit = replicaStateChangeLock synchronized {
    allPartitions.put(tp, HostedPartitionZk.Offline)
    ...
    requestedEpochInfo.map { offsetForLeaderTopic =>
      val partitions = offsetForLeaderTopic.partitions.asScala.map { offsetForLeaderPartition =>
        ...
        getPartition(tp) match {
          ...
          case HostedPartitionZk.None if metadataCache.contains(tp) =>
            new EpochEndOffset()
              .setPartition(offsetForLeaderPartition.partition)
