...
import kafka.testkit.junit.ClusterConfig.Type
import kafka.testkit.junit.{ClusterForEach, ClusterInstance}
import kafka.testkit.junit.annotations.{ClusterTest, ClusterTestDefaults}
  ...
  def testAlterIpQuotasRequest(): Unit = {
    ...
    def verifyIpQuotas(entityFilter: ClientQuotaFilterComponent, expectedMatches: Map[ClientQuotaEntity, Double]): Unit = {
      ...
      result.asScala.foreach { case (entity, props) =>
        ...
        assertEquals(expectedMatches(entity), cluster.brokers().asScala.head.connectionQuotas.connectionRateForIp(entityIp), 0.01)
      }
    }
