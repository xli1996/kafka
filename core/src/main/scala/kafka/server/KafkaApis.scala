...
class KafkaApis(val requestChannel: RequestChannel,
                val requestChannelHelper: RequestChannelHelper,
  ...
  def handleLeaderAndIsrRequest(request: RequestChannel.Request): Unit = {
    ...
    requestChannelHelper.authorizeClusterOperation(request, CLUSTER_ACTION)
    if (isBrokerEpochStale(leaderAndIsrRequest.brokerEpoch)) {
      ...
      requestChannelHelper.sendResponseExemptThrottle(request, leaderAndIsrRequest.getErrorResponse(0, Errors.STALE_BROKER_EPOCH.exception))
    } else {
      val response = replicaManager.becomeLeaderOrFollower(correlationId, leaderAndIsrRequest, onLeadershipChange)
      requestChannelHelper.sendResponseExemptThrottle(request, response)
    }
  }
  ...
  def handleStopReplicaRequest(request: RequestChannel.Request): Unit = {
    ...
    if (isBrokerEpochStale(stopReplicaRequest.brokerEpoch)) {
      ...
      requestChannelHelper.sendResponseExemptThrottle(request, new StopReplicaResponse(
      ...
      requestChannelHelper.sendResponseExemptThrottle(request, new StopReplicaResponse(new StopReplicaResponseData()
        .setErrorCode(error.code)
  ...
  def handleUpdateMetadataRequest(request: RequestChannel.Request): Unit = {
    ...
    if (isBrokerEpochStale(updateMetadataRequest.brokerEpoch)) {
      ...
      requestChannelHelper.sendResponseExemptThrottle(request,
      ...
      requestChannelHelper.sendResponseExemptThrottle(request, new UpdateMetadataResponse(
  ...
  def handleOffsetCommitRequest(request: RequestChannel.Request): Unit = {
    ...
    def sendResponseCallback(commitStatus: Map[TopicPartition, Errors]): Unit = {
      ...
      requestChannelHelper.sendResponseMaybeThrottle(request, requestThrottleMs =>
        new OffsetCommitResponse(requestThrottleMs, combinedCommitStatus.asJava))
    }

    // reject the request if not authorized to the group
    if (!requestChannelHelper.authorize(request.context, READ, GROUP, offsetCommitRequest.data.groupId)) {
      val error = Errors.GROUP_AUTHORIZATION_FAILED
      val responseTopicList = OffsetCommitRequest.getErrorResponseTopics(
        offsetCommitRequest.data.topics,
        error)

      requestChannelHelper.sendResponseMaybeThrottle(request, requestThrottleMs => new OffsetCommitResponse(
  ...
  def handleProduceRequest(request: RequestChannel.Request): Unit = {
    ...
    if (produceRequest.hasTransactionalRecords) {
      val isAuthorizedTransactional = produceRequest.transactionalId != null &&
        requestChannelHelper.authorize(request.context, WRITE, TRANSACTIONAL_ID, produceRequest.transactionalId)
      if (!isAuthorizedTransactional) {
        requestChannelHelper.sendErrorResponseMaybeThrottle(request, Errors.TRANSACTIONAL_ID_AUTHORIZATION_FAILED.exception)
        return
      }
      // Note that authorization to a transactionalId implies ProducerId authorization

    } else if (produceRequest.hasIdempotentRecords && !requestChannelHelper.authorize(request.context, IDEMPOTENT_WRITE, CLUSTER, CLUSTER_NAME)) {
      requestChannelHelper.sendErrorResponseMaybeThrottle(request, Errors.CLUSTER_AUTHORIZATION_FAILED.exception)
      return
    }
    ...
    def sendResponseCallback(responseStatus: Map[TopicPartition, PartitionResponse]): Unit = {
      ...
      if (produceRequest.acks == 0) {
        ...
        if (errorInResponse) {
          ...
          requestChannelHelper.sendNoOpResponseExemptThrottle(request)
        }
      } else {
  ...
  def handleFetchRequest(request: RequestChannel.Request): Unit = {
    ...
    def processResponseCallback(responsePartitionData: Seq[(TopicPartition, FetchPartitionData)]): Unit = {
      ...
      if (fetchRequest.isFromFollower) {
        ...
        requestChannelHelper.sendResponseExemptThrottle(request, createResponse(0), Some(updateConversionStats))
      } else {
  ...
  def handleListOffsetRequest(request: RequestChannel.Request): Unit = {
    ...
    requestChannelHelper.sendResponseMaybeThrottle(request, requestThrottleMs => new ListOffsetResponse(new ListOffsetResponseData()
      .setThrottleTimeMs(requestThrottleMs)
  ...
  def handleTopicMetadataRequest(request: RequestChannel.Request): Unit = {
    ...
    if (authorizedTopics.nonEmpty) {
      ...
      if (metadataRequest.allowAutoTopicCreation && config.autoCreateTopicsEnable && nonExistingTopics.nonEmpty) {
        if (!requestChannelHelper.authorize(request.context, CREATE, CLUSTER, CLUSTER_NAME, logIfDenied = false)) {
    ...
    if (request.header.apiVersion >= 8) {
      ...
      if (metadataRequest.data.includeClusterAuthorizedOperations) {
        if (requestChannelHelper.authorize(request.context, DESCRIBE, CLUSTER, CLUSTER_NAME))
          clusterAuthorizedOperations = requestChannelHelper.authorizedOperations(request, Resource.CLUSTER)
        else
      ...
      if (metadataRequest.data.includeTopicAuthorizedOperations) {
        topicMetadata.foreach { topicData =>
          topicData.setTopicAuthorizedOperations(requestChannelHelper.authorizedOperations(request, new Resource(ResourceType.TOPIC, topicData.name)))
        }
      }
    }
  ...
  def handleOffsetFetchRequest(request: RequestChannel.Request): Unit = {
    ...
    def createResponse(requestThrottleMs: Int): AbstractResponse = {
      val offsetFetchResponse =
        // reject the request if not authorized to the group
        if (!requestChannelHelper.authorize(request.context, DESCRIBE, GROUP, offsetFetchRequest.groupId))
    ...
    requestChannelHelper.sendResponseMaybeThrottle(request, createResponse)
  }
  ...
  def handleFindCoordinatorRequest(request: RequestChannel.Request): Unit = {
    ...
    if (findCoordinatorRequest.data.keyType == CoordinatorType.GROUP.id &&
        !requestChannelHelper.authorize(request.context, DESCRIBE, GROUP, findCoordinatorRequest.data.key))
      requestChannelHelper.sendErrorResponseMaybeThrottle(request, Errors.GROUP_AUTHORIZATION_FAILED.exception)
    else if (findCoordinatorRequest.data.keyType == CoordinatorType.TRANSACTION.id &&
        !requestChannelHelper.authorize(request.context, DESCRIBE, TRANSACTIONAL_ID, findCoordinatorRequest.data.key))
      requestChannelHelper.sendErrorResponseMaybeThrottle(request, Errors.TRANSACTIONAL_ID_AUTHORIZATION_FAILED.exception)
    else {
  ...
  def handleDescribeGroupRequest(request: RequestChannel.Request): Unit = {
    ...
    describeRequest.data.groups.forEach { groupId =>
      if (!requestChannelHelper.authorize(request.context, DESCRIBE, GROUP, groupId)) {
        ...
        if (request.header.apiVersion >= 3) {
          if (error == Errors.NONE && describeRequest.data.includeAuthorizedOperations) {
            describedGroup.setAuthorizedOperations(requestChannelHelper.authorizedOperations(request, new Resource(ResourceType.GROUP, groupId)))
          }
        }
  ...
  def handleListGroupsRequest(request: RequestChannel.Request): Unit = {
    ...
    if (requestChannelHelper.authorize(request.context, DESCRIBE, CLUSTER, CLUSTER_NAME))
      ...
      val filteredGroups = groups.filter(group => requestChannelHelper.authorize(request.context, DESCRIBE, GROUP, group.groupId))
      requestChannelHelper.sendResponseMaybeThrottle(request, requestThrottleMs =>
  ...
  def handleJoinGroupRequest(request: RequestChannel.Request): Unit = {
    ...
    } else if (!requestChannelHelper.authorize(request.context, READ, GROUP, joinGroupRequest.data.groupId)) {
  ...
  def handleSyncGroupRequest(request: RequestChannel.Request): Unit = {
    ...
    } else if (!requestChannelHelper.authorize(request.context, READ, GROUP, syncGroupRequest.data.groupId)) {
  ...
  def handleHeartbeatRequest(request: RequestChannel.Request): Unit = {
    ...
    } else if (!requestChannelHelper.authorize(request.context, READ, GROUP, heartbeatRequest.data.groupId)) {
      requestChannelHelper.sendResponseMaybeThrottle(request, requestThrottleMs =>
  ...
  def handleLeaveGroupRequest(request: RequestChannel.Request): Unit = {
    ...
    if (!requestChannelHelper.authorize(request.context, READ, GROUP, leaveGroupRequest.data.groupId)) {
      requestChannelHelper.sendResponseMaybeThrottle(request, requestThrottleMs => {
  ...
  def handleSaslHandshakeRequest(request: RequestChannel.Request): Unit = {
    ...
    requestChannelHelper.sendResponseMaybeThrottle(request, _ => new SaslHandshakeResponse(responseData))
  }
  ...
  def handleSaslAuthenticateRequest(request: RequestChannel.Request): Unit = {
    ...
    requestChannelHelper.sendResponseMaybeThrottle(request, _ => new SaslAuthenticateResponse(responseData))
  }
  ...
  def handleApiVersionsRequest(request: RequestChannel.Request): Unit = {
    ...
    requestChannelHelper.sendResponseMaybeThrottle(request, createResponseCallback)
  }
  ...
  def handleCreateTopicsRequest(request: RequestChannel.Request): Unit = {
    ...
    def sendResponseCallback(results: CreatableTopicResultCollection): Unit = {
      ...
      requestChannelHelper.sendResponseMaybeThrottle(controllerMutationQuota, request, createResponse, onComplete = None)
    }
    ...
    if (!controller.isActive) {
      ...
      val hasClusterAuthorization = requestChannelHelper.authorize(request.context, CREATE, CLUSTER, CLUSTER_NAME,
        logIfDenied = false)
  ...
  def handleInitProducerIdRequest(request: RequestChannel.Request): Unit = {
    ...
    if (transactionalId != null) {
      if (!requestChannelHelper.authorize(request.context, WRITE, TRANSACTIONAL_ID, transactionalId)) {
        requestChannelHelper.sendErrorResponseMaybeThrottle(request, Errors.TRANSACTIONAL_ID_AUTHORIZATION_FAILED.exception)
        return
      }
    } else if (!requestChannelHelper.authorize(request.context, IDEMPOTENT_WRITE, CLUSTER, CLUSTER_NAME)) {
      requestChannelHelper.sendErrorResponseMaybeThrottle(request, Errors.CLUSTER_AUTHORIZATION_FAILED.exception)
      return
    }
    ...
    producerIdAndEpoch match {
      ...
      case Left(error) => requestChannelHelper.sendErrorResponseMaybeThrottle(request, error.exception)
    }
  }
  ...
  def handleWriteTxnMarkersRequest(request: RequestChannel.Request): Unit = {
    ...
    if (numAppends.get == 0) {
      requestChannelHelper.sendResponseExemptThrottle(request, new WriteTxnMarkersResponse(errors))
      return
    }
    ...
    if (!requestChannelHelper.authorize(request.context, WRITE, TRANSACTIONAL_ID, transactionalId))
      requestChannelHelper.sendResponseMaybeThrottle(request, requestThrottleMs =>
        new AddOffsetsToTxnResponse(new AddOffsetsToTxnResponseData()
          .setErrorCode(Errors.TRANSACTIONAL_ID_AUTHORIZATION_FAILED.code)
          .setThrottleTimeMs(requestThrottleMs)))
    else if (!requestChannelHelper.authorize(request.context, READ, GROUP, groupId))
      requestChannelHelper.sendResponseMaybeThrottle(request, requestThrottleMs =>
        new AddOffsetsToTxnResponse(new AddOffsetsToTxnResponseData()
          .setErrorCode(Errors.GROUP_AUTHORIZATION_FAILED.code)
          .setThrottleTimeMs(requestThrottleMs))
  ...
  def handleTxnOffsetCommitRequest(request: RequestChannel.Request): Unit = {
    ...
    if (!requestChannelHelper.authorize(request.context, WRITE, TRANSACTIONAL_ID, txnOffsetCommitRequest.data.transactionalId))
      requestChannelHelper.sendErrorResponseMaybeThrottle(request, Errors.TRANSACTIONAL_ID_AUTHORIZATION_FAILED.exception)
    else if (!requestChannelHelper.authorize(request.context, READ, GROUP, txnOffsetCommitRequest.data.groupId))
      requestChannelHelper.sendErrorResponseMaybeThrottle(request, Errors.GROUP_AUTHORIZATION_FAILED.exception)
    else {
      val unauthorizedTopicErrors = mutable.Map[TopicPartition, Errors]()
      val nonExistingTopicErrors = mutable.Map[TopicPartition, Errors]()
  ...
  def handleDescribeAcls(request: RequestChannel.Request): Unit = {
    requestChannelHelper.authorizeClusterOperation(request, DESCRIBE)
  ...
  def handleCreateAcls(request: RequestChannel.Request): Unit = {
    requestChannelHelper.authorizeClusterOperation(request, ALTER)
    val createAclsRequest = request.body[CreateAclsRequest]

    authorizer match {
      case None => requestChannelHelper.sendResponseMaybeThrottle(request, requestThrottleMs =>
  ...
  def handleOffsetForLeaderEpochRequest(request: RequestChannel.Request): Unit = {
    ...
    val (authorizedPartitions, unauthorizedPartitions) =
      if (requestChannelHelper.authorize(request.context, CLUSTER_ACTION, CLUSTER, CLUSTER_NAME, logIfDenied = false))
  ...
  def handleAlterConfigsRequest(request: RequestChannel.Request): Unit = {
    ...
    val (authorizedResources, unauthorizedResources) = alterConfigsRequest.configs.asScala.toMap.partition { case (resource, _) =>
      resource.`type` match {
        ...
        case ConfigResource.Type.BROKER =>
          requestChannelHelper.authorize(request.context, ALTER_CONFIGS, CLUSTER, CLUSTER_NAME)
        case ConfigResource.Type.TOPIC =>
          requestChannelHelper.authorize(request.context, ALTER_CONFIGS, TOPIC, resource.name)
    ...
    requestChannelHelper.sendResponseMaybeThrottle(request, responseCallback)
  }
  ...
  def handleDescribeConfigsRequest(request: RequestChannel.Request): Unit = {
    ...
    val (authorizedResources, unauthorizedResources) = describeConfigsRequest.data.resources.asScala.toBuffer.partition { resource =>
      ConfigResource.Type.forId(resource.resourceType) match {
        case ConfigResource.Type.BROKER | ConfigResource.Type.BROKER_LOGGER =>
          requestChannelHelper.authorize(request.context, DESCRIBE_CONFIGS, CLUSTER, CLUSTER_NAME)
        case ConfigResource.Type.TOPIC =>
          requestChannelHelper.authorize(request.context, DESCRIBE_CONFIGS, TOPIC, resource.resourceName)
  ...
  def handleAlterReplicaLogDirsRequest(request: RequestChannel.Request): Unit = {
    ...
    if (requestChannelHelper.authorize(request.context, ALTER, CLUSTER, CLUSTER_NAME)) {
  ...
  def handleDescribeLogDirsRequest(request: RequestChannel.Request): Unit = {
    ...
    requestChannelHelper.sendResponseMaybeThrottle(request, throttleTimeMs => new DescribeLogDirsResponse(new DescribeLogDirsResponseData()
  ...
  def handleDescribeTokensRequest(request: RequestChannel.Request): Unit = {
    ...
    if (!allowTokenRequests(request))
      ...
      if (describeTokenRequest.ownersListEmpty()) {
        ...
        def authorizeToken(tokenId: String) = requestChannelHelper.authorize(request.context, DESCRIBE, DELEGATION_TOKEN, tokenId)
  ...
  def handleOffsetDeleteRequest(request: RequestChannel.Request): Unit = {
    ...
    if (requestChannelHelper.authorize(request.context, DELETE, GROUP, groupId)) {
  ...
  def handleDescribeClientQuotasRequest(request: RequestChannel.Request): Unit = {
    ...
    if (requestChannelHelper.authorize(request.context, DESCRIBE_CONFIGS, CLUSTER, CLUSTER_NAME)) {
  ...
  def handleAlterClientQuotasRequest(request: RequestChannel.Request): Unit = {
    ...
    if (requestChannelHelper.authorize(request.context, ALTER_CONFIGS, CLUSTER, CLUSTER_NAME)) {
  ...
  def handleAlterUserScramCredentialsRequest(request: RequestChannel.Request): Unit = {
    ...
    } else if (requestChannelHelper.authorize(request.context, ALTER, CLUSTER, CLUSTER_NAME)) {
  ...
  def handleAlterIsrRequest(request: RequestChannel.Request): Unit = {
    ...
    if (!requestChannelHelper.authorize(request.context, CLUSTER_ACTION, CLUSTER, CLUSTER_NAME)) {
      ...
      controller.alterIsrs(alterIsrRequest.data,
        alterIsrResp => requestChannelHelper.sendResponseMaybeThrottle(request, requestThrottleMs =>
