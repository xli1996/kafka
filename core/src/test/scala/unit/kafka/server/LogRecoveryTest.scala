...
class LogRecoveryTest extends ZooKeeperTestHarness {
  ...
  var server1: LegacyBroker = null
  var server2: LegacyBroker = null
  ...
  def testHWCheckpointNoFailuresSingleLogSegment(): Unit = {
    ...
    TestUtils.waitUntilTrue(() =>
      server2.replicaManager.localOnlineLogOrException(topicPartition).highWatermark == numMessages,
  ...
  def testHWCheckpointWithFailuresSingleLogSegment(): Unit = {
    ...
    /** We plan to shutdown server2 and transfer the leadership to server1.
      ...
      */
    waitUntilTrue(() => server2.replicaManager.deferredOrOnlinePartition(topicPartition).get.inSyncReplicaIds.size == 2,
    ...
    TestUtils.waitUntilTrue(() =>
      server2.replicaManager.localOnlineLogOrException(topicPartition).highWatermark == hw,
      "Failed to update high watermark for follower after timeout")
  ...
  def testHWCheckpointWithFailuresMultipleLogSegments(): Unit = {
    ...
    TestUtils.waitUntilTrue(() => server1.replicaManager.localOnlineLog(topicPartition).nonEmpty,
      "Failed to create replica in follower after timeout")
    // allow some time for the follower to get the leader HW
    TestUtils.waitUntilTrue(() =>
      server1.replicaManager.localOnlineLogOrException(topicPartition).highWatermark == hw,
      "Failed to update high watermark for follower after timeout")
    // shutdown the servers to allow the hw to be checkpointed
    servers.foreach(_.shutdown())
    assertEquals(hw, hwFile1.read().getOrElse(topicPartition, 0L))
